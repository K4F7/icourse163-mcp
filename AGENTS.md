...
- Homework/quiz assist pipeline: `get_homework` (read) → AI fill → **`save_homework_answers` REQUIRED (draft, preview=true)** → user confirms → explicit `submit_homework` (preview=false). Never implicit submit. Exam 代交 out of scope (`submit_homework` refuses exam todo ids).
